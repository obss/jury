...
def test_basic(predictions, references, jury_meteor, output_basic):
    scores = jury_meteor(predictions=predictions, references=references)
    assert_almost_equal_dict(actual=scores, desired=output_basic)


def test_multiple_ref(predictions, multiple_references, jury_meteor, output_multiple_ref):
    scores = jury_meteor(predictions=predictions, references=multiple_references)
    assert_almost_equal_dict(actual=scores, desired=output_multiple_ref)


def test_multiple_pred_multiple_ref(
    multiple_predictions, multiple_references, jury_meteor, output_multiple_pred_multiple_ref
):
    scores = jury_meteor(predictions=multiple_predictions, references=multiple_references)

...
def jury_f1_language_generation():
    metric = AutoMetric.load("f1")
    return Jury(metrics=metric)


@pytest.fixture(scope="module")
def jury_f1_sequence_classification():
...
def test_basic_language_generation(
    predictions, references, jury_f1_language_generation, output_basic_language_generation
):
    scores = jury_f1_language_generation(predictions=predictions, references=references)
    assert_almost_equal_dict(actual=scores, desired=output_basic_language_generation)


def test_multiple_ref_language_generation(
    predictions, multiple_references, jury_f1_language_generation, output_multiple_ref_language_generation
):
    scores = jury_f1_language_generation(predictions=predictions, references=multiple_references)
...
def test_multiple_pred_multiple_ref_language_generation(
    ...
    multiple_references,
    jury_f1_language_generation,
    output_multiple_pred_multiple_ref_language_generation,
):
    scores = jury_f1_language_generation(predictions=multiple_predictions, references=multiple_references)
...
def test_basic_sequence_classification(
    ...
    jury_f1_sequence_classification,
    output_basic_sequence_classification,
):
    scores = jury_f1_sequence_classification(

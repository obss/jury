...
def test_evaluate_concurrent(predictions, references, jury_concurrent):
    scores = jury_concurrent.evaluate(predictions=predictions, references=references)
...
def test_evaluate_no_input(predictions, references, jury):
    with pytest.raises(TypeError):
        jury.evaluate(predictions=predictions)
        jury.evaluate(references=references)
        jury.evaluate()
...
def test_evaluate_inconsistent_input(inconsistent_predictions, references, jury):
    ...
    with pytest.raises(ValueError):
        jury.evaluate(predictions=inconsistent_predictions, references=references)
...
def test_evaluate_basic_str_input(predictions, references, jury_str):
    scores = jury_str.evaluate(predictions=predictions, references=references)
...
def test_reduce_fn(predictions, references, jury):
    ...
    scores = jury.evaluate(predictions=predictions, references=references, reduce_fn=_reduce_fn)

    assert all([scores[metric.resulting_name] is not None for metric in jury.metrics])

    with pytest.raises(ValueError):
        jury.evaluate(predictions=predictions, references=references, reduce_fn=_non_reduce_fn)
